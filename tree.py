...
class DecisionTree:
    ...
    def split_graph(self, feature_column, threshold):
        ...
        for class_label in range(self.num_classes):
            class_indices = np.where(feature_column >= threshold)[0]
    ...
    def build_tree(self, X, y):
        ...
        # return a leaf with the most common label 
            ...
            split_indices = self.split_graph(X[:, best_split_feature], best_threshold)
            split_subtrees = []

            for class_label, indices in enumerate(split_indices):
                if len(indices) == 0:
                    most_common_label = np.bincount(y).argmax()
                    subtree = {'leaf_node': most_common_label}
                else:
                    subtree = self.build_tree(X[indices], y[indices])
                split_subtrees.append(subtree)
    ...
    def predict_single(self, x, node = None):
        ...
        for subtree in node['split_subtrees']:
